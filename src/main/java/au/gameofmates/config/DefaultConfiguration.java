...
public class DefaultConfiguration {
  ...
  public List<LoadableGraphSpec> vertexesToLoad() {
    ...
    try {
      ...
      logger.error(
          "Could not read the vertexLoad specification specified by attribute `au.gameofmates.loadspec`",
          e);
  ...
  public List<RelationshipGraphSpec> edgesToLoad() {
    ...
    try {
      json = FileCopyUtils.copyToString(new InputStreamReader(resource.getInputStream()));
      List<RelationshipGraphSpec> ppl2 =
          Arrays.asList(objectMapper.readValue(json, RelationshipGraphSpec[].class));

      logger.info("Load record of " + ppl2.size() + " relationship record elements.");
      return ppl2;
    } catch (IOException e) {
      logger.error(
          "Could not read the edgeLoad specification specified by attribute `au.gameofmates.loadspec`",
          e);
      throw new RuntimeException(e);
    }
  ...
  @Bean
  public Graph initialiseGraph(List<LoadableGraphSpec> specsToLoad,
  ...
  protected void loadRelationships(Graph g, List<RelationshipGraphSpec> rels) {
    for (RelationshipGraphSpec rgs : rels) {
      ...
      for (List<Object> relValues : arrayCSV) {
        ...
        g.traversal().V().filter(t -> t.get().value("id").equals(subjID))
            .forEachRemaining(y -> vertexList.add(y));
        g.traversal().V().filter(t -> t.get().value("id").equals(objID))
            .forEachRemaining(y -> vertexList.add(y));

        if (vertexList.size() < 2) {
          logger.error(
              "Could not find Vertices for edge between subject: " + subjID + " object: " + objID);
          throw new RuntimeException(
              "Invalid relationship specification for subject " + subjID + " Object: " + objID);
        }

        Edge edge =
            vertexList.get(0).addEdge(rgs.getLabel(), vertexList.get(1), relValues.toArray());
        logger.info("Added Edge " + edge.label() + " " + edge.outVertex().value("id") + " between "
            + edge.inVertex().value("id"));
      }
